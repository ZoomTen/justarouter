...
up generating, for each time this macro is invoked. The output will look
something like this:
...
============= DYNAMIC ROUTES =============
/users/{id}/image/{imageId}/{token}:
  GET => "\noutput = \"get params: [\" & getParams & \"], id: \" & pathParams[\"id\"] &\n    \", image: \" &\n    pathParams[\"imageId\"] &\n    \", token: \" &\n    pathParams[\"token\"]"
...
============= DYNAMICROUTE NAMES ============
R0 => /users/{id}/image/{imageId}/{token}
R1 => /users/{id}/profile
============= GENERATED REGEX PATTERN =============
(?P<R0>/users/(?P<R0_id>[^/]+?)/image/(?P<R0_imageId>[^/]+?)/(?P<R0_token>[^/]+?)$)|(?P<R1>/users/(?P<R1_id>[^/]+?)/profile$)
======BUILT=========
proc testRoute(reqType, path: string; params: ProbablyServerState;
               output: var string): void =
  try:
    template notFound(reqType, pathOnly, getParams: string;
                      pathParams: StringTableRef; params: ProbablyServerState;
                      output: var string): void =
      output = "not found"

    template methodNotAllowed(reqType, pathOnly, getParams: string;
                              pathParams: StringTableRef;
                              params: ProbablyServerState; output: var string): void =
      output = "method not allowed"

    var pathParams = newStringTable()
    var startPos`gensym23 = -1
    for i`gensym23 in 0 ..< len(path):
      case path[i`gensym23]
      of '?', '&':
        startPos`gensym23 = i`gensym23
        break
      else:
        discard
    let pathOnly = block:
      var p`gensym23 = if startPos`gensym23 > -1:
        path[0 ..< startPos`gensym23] else:
        path
      ## Clean up trailing slash
      ## 
      if len(p`gensym23) > 1 and p`gensym23[^1] == '/':
        p`gensym23[0 ..^ 2]
      else:
        p`gensym23
    let getParams = block:
      if startPos`gensym23 > -1:
        path[(startPos`gensym23 + 1) ..^ 1]
      else:
        ""
    case pathOnly
    of "/api":
      ...
      of "GET":
        ...
        methodNotAllowed(reqType, pathOnly, getParams, pathParams, params,
                         output)
    of "/":
      case reqType
      of "GET":
    ...
    of "/about":
      ...
      const
        rexp`gensym22 = re2"(?P<R0>/users/(?P<R0_id>[^/]+?)/image/(?P<R0_imageId>[^/]+?)/(?P<R0_token>[^/]+?)$)|(?P<R1>/users/(?P<R1_id>[^/]+?)/profile$)"
      var m`gensym22: RegexMatch2
      if match(pathOnly, rexp`gensym22, m`gensym22):
        let groupNames`gensym22 = groupNames(m`gensym22)
        var matchedRoute_520093999 = ""
        for i`gensym22 in 0 ..< 2:
          let routeNameCandidate`gensym22 = "R" & $i`gensym22
          try:
            if m`gensym22.group(routeNameCandidate`gensym22) != reNonCapture:
              matchedRoute_520093999 = routeNameCandidate`gensym22
              break
          except KeyError:
            notFound(reqType, pathOnly, getParams, pathParams, params, output)
            return
        if len(matchedRoute_520093999) > 1:
          for i`gensym22 in groupNames`gensym22:
            if (i`gensym22.startsWith(matchedRoute_520093999) and
                len(i`gensym22) > len(matchedRoute_520093999)):
              try:
                pathParams[i`gensym22[(len(matchedRoute_520093999) + 1) ..^ 1]] = pathOnly[
                    m`gensym22.group(i`gensym22)]
                discard
              except KeyError:
                discard
          case matchedRoute_520093999
          of "R0":
            case reqType
            of "GET":
              output = "get params: [" & getParams & "], id: " &
                  pathParams["id"] &
                  ", image: " &
                  pathParams["imageId"] &
                  ", token: " &
